    ...
    pub fn set_logger(&mut self, logger: Option<Logger>) {
        ...
        let c_logger = if let Some(logger) = logger {
            ...
            unsafe extern "C" fn log(
                ...
                let callback = unsafe { payload.cast::<Logger>().as_mut().unwrap() };
                if let Ok(message) = unsafe { CStr::from_ptr(c_message) }.to_str() {
...
pub unsafe fn set_allocator(
    ...
    unsafe { FREE_FN = new_free.unwrap_or(free) };
    unsafe { ffi::ts_set_allocator(new_malloc, new_calloc, new_realloc, new_free) };
}
