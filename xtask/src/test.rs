...
pub fn run(args: &Test) -> Result<()> {
    ...
    if let Some(language) = &args.language {
        // SAFETY: This is called at the start of the program before any threads are spawned.
        unsafe { env::set_var("TREE_SITTER_LANGUAGE", language) };
    }
    if let Some(example) = &args.example {
        // SAFETY: This is called at the start of the program before any threads are spawned.
        unsafe { env::set_var("TREE_SITTER_EXAMPLE_INCLUDE", example) };
    }
    if let Some(seed) = args.seed {
        // SAFETY: This is called at the start of the program before any threads are spawned.
        unsafe { env::set_var("TREE_SITTER_SEED", seed.to_string()) };
    }
    if let Some(iterations) = args.iterations {
        // SAFETY: This is called at the start of the program before any threads are spawned.
        unsafe { env::set_var("TREE_SITTER_ITERATIONS", iterations.to_string()) };
    }
    if args.debug {
        // SAFETY: This is called at the start of the program before any threads are spawned.
        unsafe { env::set_var("TREE_SITTER_LOG", "1") };
    }
    if args.debug_graph {
        // SAFETY: This is called at the start of the program before any threads are spawned.
        unsafe { env::set_var("TREE_SITTER_LOG_GRAPHS", "1") };
    }
