    ...
    for payload in parser.parse_all(&wasm_bytes) {
        if let wasmparser::Payload::ImportSection(imports) = payload? {
            for import_group in imports {
                for result in import_group? {
                    let (_, import) = result?;
                    if !builtin_symbols.contains(&import.name)
                        && !stdlib_symbols.contains(&import.name)
                        && !dylink_symbols.contains(&import.name)
                    {
                        missing_symbols.push(import.name);
                    }
                }
            }
        }
